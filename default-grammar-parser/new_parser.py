...
"""ToDo:
    Documentation -docstrings and individual lines
    error handling
    organization
    clean code -
        variable names
        self descriptive pythonic code
        function definition locations
        easily understood algorithms
    testing - try to break it

    FixMe:
        grammar_table holds popo numbers - better way to do this?
        grammar table to dict is done messily. grammar_table.csv stored poorly (transpose function?)
        there are some unused variables (delete them - check why they exist)
        R0 (S`) -> I believe this will still break the program if we reach it.
